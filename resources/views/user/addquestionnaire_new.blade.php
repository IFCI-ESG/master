...
<style>
    ...
    .important-notes h2 {
        font-size: 1.5em;
        margin-bottom: 10px;
        color: #d9534f; /* Optional: gives a red alert color */
    }
    ...
    <div class="col-lg-8 offset-md-2">
        ...
        <div class="card card-success card-outline mt-5 ml-2" style="box-shadow: 0 4px 10px 0 rgba(182, 233, 152, 0.474), 0 5px 20px 0 rgba(182, 233, 152, 0.474);">
            <div class="card-header">
                <b class="table-heading-sec">Business Activity </b>
            </div>
            ...
            <div class="card card-success card-outline card-tabs shadow-lg">
                <div class="card-header p-0 pt-3 border-bottom-0">
                    <ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
                        <li class="nav-item">
                            ...
                            role="tab" aria-controls="environment" aria-selected="true"><b class="table-heading-sec">{{$sector->name}} Sector Environment Data For FY-{{$fys->fy}}</b></a>
                        </li>
                    </ul>
                </div>
                <form action="{{ route('user.questionnaire.quality_store') }}" id="" role="form" method="post"
                class='form-horizontal prevent_multiple_submit' files=true enctype='multipart/form-data'
                accept-charset="utf-8">
                @csrf
                ...
                <div class="card-body">
                    <div class="tab-content" id="custom-tabs-three-tabContent">
                        <div class="tab-pane fade show active" id="environment" role="tabpanel"  aria-labelledby="environment-tab">
                            ...
                            <div class="card">
                                <div class="card-body p-3">
                                    <table class="table table-bordered  table-sm table-striped projects" id="env-table">
                                        <thead>
                                            <tr class="text-center table-success">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="hidden" value="{{ $data_qual_value->where('segment_id', $seg->id)->first()->id }}" name="part[{{ $key }}][row_id]">
                                                    <select name="part[{{ $key }}][data_quality_id]" id="quality" class="form-control form-control-sm">
                                                        @foreach ($data_quality as $data)
                                                        <option value="{{ $data->id }}" {{ $data->id == $data_qual_value->where('segment_id', $seg->id)->first()->data_quality_id ? 'selected' : '' }}>{{ $data->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                @else
                ...
                <div class="row pb-2 mt-2 d-flex align-items-center">
                    <div class="col-md-2 ml-4">
                        <a href="{{ route('user.fy',['bank_id' => encrypt($bank_details->bank_id),'class_type'=> encrypt($bank_details->class_type_id)]) }}"
                        ...
                        <div class="col-md-2 offset-md-2">
                            @if(!$busi_value->isEmpty())
                            <button type="submit" id="final_submit" class="btn btn-primary btn-sm form-control form-control-sm Custom-btn"><i class="fas fa-save"></i>
                                @if (!$data_qual_value->isEmpty()) Update @else Save as Draft @endif</button>
                                @endif
...
<div class="container py-4 px-4 col-lg-8" style="background-color: #f9fafc; border: 1px solid #d1d5db; border-radius: 8px; box-shadow: 0px 4px 12px rgba(0, 0, 0, 0.1);">
...
@include('partials.user.environment.popup_new')
@include('partials.user.environment.popup_view_new')
<script>
