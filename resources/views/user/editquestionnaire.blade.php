                                    ...
                                    <div class="nav flex-column nav-tabs h-100" id="vert-tabs-tab" role="tablist"
                                        ...
                                        @foreach ($segments as $key => $seg)
                                            <a @if ($seg->id == 1) class="nav-link active shadow" @else class="nav-link shadow" @endif
                                                id="vert-tabs-{{ $seg->id }}-tab" data-toggle="pill"
                                                href="#vert-tabs-{{ $seg->id }}" role="tab"
                                                aria-controls="vert-tabs-{{ $seg->id }}"
                                                aria-selected="{{ $seg->id == 1 ? 'true' : 'false' }}">{{ $seg->segment_name }}
                                            </a>
                                        @endforeach
                                    </div>
                                </div>
                                    ...
                                    <div class="tab-content" id="vert-tabs-tabContent">
                                        ...
                                        @foreach ($segments as $key => $seg)
                                            <div @if ($seg->id == 1) class="tab-pane text-left fade show active" @else class="tab-pane fade" @endif
                                                ...
                                                id="vert-tabs-{{ $seg->id }}" role="tabpanel"
                                                aria-labelledby="vert-tabs-{{ $seg->id }}-tab">
                                                        ...
                                                        @foreach ($questions->where('status', '1')->where('seg_id', $seg->id) as $key => $ques)
                                                            <tr>
                                                                <td style="font-size: 1rem;">
                                                                    {{ $ques->particular }}
                                                                    <input type="hidden" value="{{ $ques->ques_id }}"
                                                                        name="part[{{ $key }}][ques_id]">
                                                                    <input type="hidden" value="{{ $ques->row_id }}"
                                                                        ...
                                                                        <div>
                                                                            ...
                                                                            <label>
                                                                                <input type="radio"
                                                                                    name="part[{{ $key }}][value]"
                                                                                    value="0"
                                                                                    @if ($ques->value == '0') checked @endif
                                                                                    class="form-control form-control-sm text-right">
                                                                                No
                                                                            </label>
                                                                        </div>
                                                                    @else
                                                                        <input type="{{ $ques->data_type }}" readonly
                                                                            required value="{{ $ques->value }}"
                                                                            name="part[{{ $key }}][value]"
                                                                            class="form-control form-control-sm text-right"
                                                                            style="width:100%">
                                                                    @endif
