...
class UserController extends Controller
{
    public function fy()
    {
        ...
        return view('user.fy', compact('fys','ques','input_mast','user'));
    ...
    public function addquestionnaire($fy_id)
    {
        ...
        return view('user.addquestionnaire', compact('ques_mast','fy_id','user','ques_value','sector','fys','seg_mast','busi_mast','busi_value'));
    ...
    public function print_preview($com_id,$fy_id)
    {
        $fy_id = decrypt($fy_id);
        ...
        // dd($busi_acti,$ques_val,$seg_mast);

        return view('user.preview', compact('ques_val','seg_mast','busi_acti','input_mast','user'));
...
// dd($request->undertaking,"d");
            DB::transaction(function () use ($request, $user)
            {
                $input_mast = InputSheetMast::where('id', $request->input_id)->first();
                    $input_mast->status = 'S';
                    $input_mast->is_checked = isset($value['undertaking']) ? 1 : 0;
    ...
    public function view($com_id,$fy_id)
    {
        ...
        return view('user.questionnaire_view', compact('ques_mast','fy_id','user','ques_value','sector','fys','seg_mast','busi_mast','busi_value'));
